...
// case sensitive
func Test_case_sensitive_field(t *testing.T){
	should := require.New(t)
	type product struct {
		Name string
		ProductID int64
	...
	var str = `{"name":"Xiao mi 6","nmber":10000,"price":2499,"is_on_sale": false}`
	config := jsoniter.Config{CaseSensitive: true}.Froze()
	var gval product
	err := config.UnmarshalFromString(str,&gval)
...
// only tag field
func Test_encode_only_tag_filed(t *testing.T){
	should := require.New(t)
	type product struct {
	...
	var str = `{"name":"Xiao mi 6","number":10000,"price":2499,"is_on_sale": false}`
	config := jsoniter.Config{OnlyTaggedField: false}.Froze()
	var gval product
	err := config.UnmarshalFromString(str,&gval)
	fmt.Println(str,"\nthe unmarshal for string result is ", gval)

	//str1, err := config.MarshalToString(group)
	//fmt.Println(str,"\nthe marshal result is ", str1)
	should.Nil(err)
}

// SortMapKeys
func Test_encode_sort_key_map(t *testing.T){
	should := require.New(t)
	var val map[*big.Float]string

	config := jsoniter.Config{SortMapKeys: true}.Froze()
	config.UnmarshalFromString(`{"4":"1", "2":"3","1":"5"}`,&val)
	str, err := config.MarshalToString(val)
	fmt.Println(str)
	should.Nil(err)
	should.Equal(`{"1":"5","2":"3","4":"1"}`, str)
}
